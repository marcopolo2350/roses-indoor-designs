...
function resolveLabel(raw) {
  ...
  const map = [
    ...
    {
      keys: ["table", "coffee table", "end table", "side table", "dining table", "console"],
      type: "table",
    },
    ...
    {
      keys: ["lamp", "floor lamp", "table lamp", "standing lamp", "light", "sconce"],
      type: "lamp",
    },
...
function inferAssetKey(label, mountType) {
  ...
  if (s.includes("wall art"))
    return ["wall_art_01", "wall_art_04", "wall_art_06"][Math.abs(hashCode(label)) % 3];
...
function defaultElevation(mountType, assetKey, type) {
  if (window.CatalogPlacementRules?.defaultElevation) {
    return window.CatalogPlacementRules.defaultElevation({
      mountType,
      assetKey,
      type,
      roomHeight: curRoom?.height,
    });
  }
  if (mountType === "ceiling") return curRoom?.height ? Math.max(7.2, curRoom.height - 0.6) : 8.4;
...
function collidesWithStructure(px, pz, room) {
  ...
  for (const st of room.structures) {
    if (st.type === "closet" && st.rect) {
      ...
      if (
        px >= st.rect.x - margin &&
        px <= st.rect.x + st.rect.w + margin &&
        py >= st.rect.y - margin &&
        py <= st.rect.y + st.rect.h + margin
      ) {
        return true;
      }
    }
    ...
    try {
      if (typeof currentFloorRooms === "function") {
        const floorRooms = currentFloorRooms(
          roomOrRooms,
          roomOrRooms.floorId || activeProjectFloorId,
        );
        if (floorRooms?.length) return floorRooms.filter((room) => room?.polygon?.length);
      }
    } catch (error) {
      window.reportRoseRecoverableError?.("Related room lookup failed", error);
    }
    return [roomOrRooms];
  }
  return [];
...
function findWalkRoomsAtPoint(px, pz, roomOrRooms = curRoom) {
  ...
  return resolveWalkRooms(roomOrRooms).filter((room) => {
    ...
    return (
      nearDoorOpening(px, pz, room, 2.5) &&
      py >= getRoomBounds2D(room).y0 - 2.5 &&
      py <= getRoomBounds2D(room).y1 + 2.5
    );
  });
}
...
function findWalkStart(roomOrRooms = curRoom) {
  ...
  const b = getWalkBounds2D(rooms);
  for (let i = 0; i < 18; i++) {
    const x = b.x0 + 0.9 + ((b.width - 1.8) * (i % 6)) / 5;
    const y = b.y0 + 0.9 + ((b.height - 1.8) * Math.floor(i / 6)) / 2;
...
function toggleMeasurements() {
  ...
  document
    .querySelectorAll(".tb")
    .forEach((b) => b.classList.toggle("on", b.dataset.t === "measure" && showMeasurements));
  draw();
}
...
function distanceInputStep(baseFeet = 0.5) {
  return unitSystem === "metric"
    ? Math.max(0.05, Number((baseFeet * FEET_TO_METERS).toFixed(2)))
    : baseFeet;
}
...
function snapFurnitureValue(value) {
  return Math.round(value / FURNITURE_GRID) * FURNITURE_GRID;
}
let lastPlanSnapState = null;
let lastFurnitureSnapState = null;
function distancePoint(a, b) {
  return window.Planner2DGeometry.distancePoint(a, b);
}
...
function resolvePlanSnap(point, room = curRoom, opts = {}) {
  ...
  points.forEach((target) => {
    ...
    if (d < best.distance && d <= pointTol)
      best = { type: "point", point: { x: target.x, y: target.y }, distance: d, target };
  });
  const intersections = [];
  ...
  intersections.forEach((target) => {
    const d = distancePoint(point, target);
    if (d < best.distance && d <= pointTol * 1.15)
      best = { type: "intersection", point: { x: target.x, y: target.y }, distance: d, target };
  });
  if (best.type === "free") {
    lines.forEach((line) => {
      const proj = closestPointOnSegment(point, line.a, line.b);
      if (proj.distance < best.distance && proj.distance <= lineTol)
        best = { type: "line", point: { x: proj.x, y: proj.y }, distance: proj.distance, line };
    });
  }
  let snapped = { x: point.x, y: point.y };
  if (best.type !== "free") snapped = { ...best.point };
  if (snapGrid) {
    const gx = snapFurnitureValue(snapped.x),
      gy = snapFurnitureValue(snapped.y);
  ...
  if (best.type === "line" && best.line)
    guides.push({ type: "segment", a: best.line.a, b: best.line.b });
  if (best.type === "point" || best.type === "intersection")
    guides.push({ type: "point", x: snapped.x, y: snapped.y });
  lastPlanSnapState = { source: best.type, guides, point: snapped };
  return {
    x: Math.round(snapped.x * 2) / 2,
    y: Math.round(snapped.y * 2) / 2,
    state: lastPlanSnapState,
  };
}
...
function snapFurniturePoint(x, z) {
  ...
  try {
    ...
    list.forEach((f, i) => {
      ...
      [f.z, f.z - hd, f.z + hd].forEach((tz) => {
        const d = Math.abs(tz - z);
        if (d < bestZ) {
          bestZ = d;
          sz = tz;
          alignedZ = true;
    ...
    window.reportRoseRecoverableError?.("Furniture alignment guide lookup failed", error);
  }
...
function updateRoomPreviewThumb(room = curRoom) {
  ...
  try {
    room.previewThumb =
      renderRoomModeToDataURL(room, "combined", 280, 180, { legend: false, measurements: false }) ||
      room.previewThumb ||
      "";
  } catch (error) {
...
function drawDimLine(ctx, ax, ay, bx, by, offset, label) {
  ...
  const nx = -dy / len,
    ...
    oy = ny * offset;
  ctx.save();
  ctx.strokeStyle = "rgba(142,110,107,.55)";
  ctx.lineWidth = 1;
  ctx.setLineDash([]);
  // extension lines
  ctx.beginPath();
  ...
  const tk = 4;
  ctx.beginPath();
  ctx.moveTo(mx - nx * tk, my - ny * tk);
  ctx.lineTo(mx + nx * tk, my + ny * tk);
  ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(ex - nx * tk, ey - ny * tk);
  ctx.lineTo(ex + nx * tk, ey + ny * tk);
  ctx.stroke();
  // label
  const cmx = (mx + ex) / 2,
    cmy = (my + ey) / 2;
  ctx.fillStyle = "rgba(142,110,107,.85)";
  ...
  ctx.rotate(ang);
  ctx.fillStyle = "rgba(250,247,242,.85)";
  ctx.fillRect(-label.length * 3.4 - 4, -8, label.length * 6.8 + 8, 16);
  ctx.fillStyle = "rgba(142,110,107,.85)";
...
function getRoomBounds2D(room) {
  ...
  (room?.furniture || []).forEach((f) => {
    ...
    [
      ...
      pts.push({
        x: f.x + dx * Math.cos(an) - dz * Math.sin(an),
        y: f.z + dx * Math.sin(an) + dz * Math.cos(an),
      });
    });
  });
  (room?.structures || []).forEach((st) => {
  ...
  if (!pts.length)
    return { x0: 0, y0: 0, x1: 12, y1: 10, width: 12, height: 10, cx: 6, cy: 5, maxD: 6 };
  let x0 = Infinity,
    y0 = Infinity,
    x1 = -Infinity,
    y1 = -Infinity;
...
function genWalls(room) {
  ...
  const prevMap = new Map(
    prev
      .map((wall) => {
        const a = room.polygon?.[wall.startIdx],
          b = room.polygon?.[wall.endIdx];
        return a && b ? [keyFor(a, b), wall] : null;
      })
      .filter(Boolean),
  );
  for (let i = 0; i < p.length; i++) {
    const n = (i + 1) % p.length,
      a = p[i],
      b = p[n],
      match = prevMap.get(keyFor(a, b));
    w.push({
      id: match?.id || uid(),
      startIdx: i,
      endIdx: n,
...
function autoSquareRoom(room, opts = {}) {
  if (!room || !Array.isArray(room.polygon) || room.polygon.length < 3)
    return { changed: false, reason: "no polygon" };
  ...
  if (poly.length >= 3) {
    ...
    if (Math.hypot(a.x - b.x, a.y - b.y) < gapTol && (a.x !== b.x || a.y !== b.y)) {
      poly[poly.length - 1] = { x: a.x, y: a.y };
      changed = true;
    }
  }
  ...
  for (let i = 0; i < poly.length; i++) {
    ...
    let best = ang,
      bd = angleTol;
    quads.forEach((q) => {
      const d = Math.abs(((ang - q + Math.PI * 3) % (Math.PI * 2)) - Math.PI);
      if (d < bd) {
        bd = d;
        best = q;
      }
    ...
    if (best !== ang) {
      const nx = a.x + Math.cos(best) * len,
        ny = a.y + Math.sin(best) * len;
      const rx = Math.round(nx * 4) / 4,
        ry = Math.round(ny * 4) / 4;
      if (Math.abs(b.x - rx) > 0.001 || Math.abs(b.y - ry) > 0.001) {
        b.x = rx;
        b.y = ry;
        changed = true;
      }
  ...
  for (let i = poly.length - 1; i >= 0 && poly.length > 3; i--) {
    ...
    if (Math.hypot(cur.x - prev.x, cur.y - prev.y) < mergeTol) {
      poly.splice(i, 1);
      changed = true;
      continue;
    }
...
function autoSquareCurrentRoom() {
  if (typeof curRoom === "undefined" || !curRoom) {
    if (typeof toast === "function") toast("Open a room first");
    return;
  }
  ...
  if (typeof toast === "function")
    toast(res.changed ? `Auto-squared room (${res.vertices} vertices)` : "Room already clean");
}
...
function findRoomsFromSegments(segments, opts = {}) {
  ...
  function onSeg(px, py, s) {
    ...
    if (t <= TOL / Math.sqrt(len2) || t >= 1 - TOL / Math.sqrt(len2)) return false;
    const cx = s.x1 + t * dx,
      cy = s.y1 + t * dy;
  ...
  const allEndpoints = [];
  segments.forEach((s) => {
    allEndpoints.push([s.x1, s.y1], [s.x2, s.y2]);
  });
  ...
  const vKey = new Map();
  function vId(x, y) {
    const kx = Math.round(x / snap),
      ky = Math.round(y / snap);
    const k = kx + "," + ky;
    if (vKey.has(k)) return vKey.get(k);
    const id = verts.length;
    verts.push({ x: kx * snap, y: ky * snap, adj: [] });
    vKey.set(k, id);
    return id;
  }
  ...
  for (let u = 0; u < verts.length; u++) {
    for (const v0 of verts[u].adj) {
      ...
      const min = Math.min(...loop),
        rot = loop.indexOf(min);
...
function splitRoomByInteriorWalls(room) {
  ...
  if (!interior.length) {
    if (typeof toast === "function") toast("No interior walls to split on");
    return { ok: false };
  }
  const segments = [];
  const p = room.polygon;
  ...
  if (sub.length < 2) {
    if (typeof toast === "function") toast("No additional rooms detected");
    return { ok: false, count: sub.length };
  }
  ...
  for (let k = 1; k < sub.length; k++) {
    ...
    base.id = uid();
    base.name = (room.name || "Room") + " " + (k + 1);
    base.polygon = sub[k];
    base.walls = genWalls(base);
    base.furniture = [];
    base.openings = [];
    base.annotations = [];
    base.textAnnotations = [];
    base.dimensionAnnotations = [];
    base.interiorWalls = [];
    if (typeof projects !== "undefined" && projects && Array.isArray(projects.rooms))
      projects.rooms.push(base);
    created.push(base);
  }
  if (typeof saveAll === "function") saveAll();
  if (typeof draw === "function") draw();
...
function wS(r, w) {
  return r.polygon[w.startIdx];
}
function wE(r, w) {
  return r.polygon[w.endIdx];
}
function wL(r, w) {
  const a = wS(r, w),
    b = wE(r, w);
