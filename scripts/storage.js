...
function openDatabase() {
  return window.RoseStorageService.openDatabase();
}
function odb() {
  return openDatabase();
}
async function getRecord(k, options) {
  return window.RoseStorageService.getRecord(k, options);
}
async function dg(k, options) {
  return getRecord(k, options);
}
async function setRecord(k, v) {
  return window.RoseStorageService.setRecord(k, v);
}
async function ds(k, v) {
  return setRecord(k, v);
}
...
function updateProfileChip() {
  ...
  if (chip)
    chip.textContent =
      PROFILE_LABELS[activeProfile] || window.APP_CONFIG?.branding?.studioLabel || "Studio";
}
function openProfileSwitcher() {
  document.getElementById("profileMod")?.classList.add("on");
}
function closeProfileSwitcher() {
  document.getElementById("profileMod")?.classList.remove("on");
}
...
const WALL_PALETTES = [
  { id: "warm_white", name: "Warm White", color: "#F3EFE7", sheen: 0.08 },
  { id: "soft_beige", name: "Soft Beige", color: "#E8D7C5", sheen: 0.09 },
  { id: "greige", name: "Greige", color: "#CFC4B6", sheen: 0.08 },
  { id: "sage", name: "Sage", color: "#BCCAB9", sheen: 0.07 },
  { id: "dusty_rose", name: "Dusty Rose", color: "#D2B0AD", sheen: 0.07 },
  { id: "muted_blue", name: "Muted Blue", color: "#AEBCCC", sheen: 0.06 },
  { id: "charcoal_accent", name: "Charcoal Accent", color: "#55525A", sheen: 0.04 },
];
...
const ROOM_TYPES = [
  ...
  },
  {
    id: "entry",
    name: "Entry / Hall",
    suggestions: ["console_low", "mirror", "runner_rug", "lamp_wall", "bench", "plant_round"],
  },
  {
    id: "kitchen",
    name: "Kitchen",
...
const DESIGN_PRESETS = [
  {
    ...
    wallFinish: "soft_beige",
    floorType: "medium_oak",
    trim: "#D8CBB8",
    lightingPreset: "warm_evening",
    ceilingBrightness: 1.05,
    mood: "elegant",
    note: "Soft oak, beige walls, and cozy evening light.",
  },
  {
    id: "quiet_luxury",
    name: "Quiet Luxury",
  ...
  {
    id: "tailored_masculine",
    name: "Tailored Masculine",
    roomType: "office",
  ...
  },
  {
    ...
    ceilingBrightness: 1.14,
    mood: "bright",
    note: "Clean light finishes and soft open daylight.",
  },
];
const ROOM_STARTERS = [
  {
...
const MODEL_REGISTRY = {
  ...
  table_round_large: {
    // table_round_large.glb was a byte-for-byte duplicate of dining_table.glb
    // (a rectangular table) — so the "Round Dining Table" catalog item was
    // rendering as a rectangle. Reuse the actually-round Polyhaven model.
    file: "ph_table_round.glb",
    category: "table",
    mountType: "floor",
    defaultScale: 1,
    defaultRotation: 0,
  ...
  ph_shelf_01: {
    ...
    yawOffset: 0,
    forwardAxis: "+z",
    wallFacingMode: "face_interior",
    defaultFacing: "interior",
  },
  ph_bookshelf: {
    file: "ph_bookshelf.glb",
    category: "bookshelf",
    mountType: "floor",
    defaultScale: 1,
    yOffset: 0,
  ...
  ph_chandelier_chinese: {
    file: "ph_chandelier_chinese.glb",
    category: "lamp",
    mountType: "ceiling",
    defaultScale: 1,
    yOffset: 0,
  ...
  ph_chandelier_lantern: {
    file: "ph_chandelier_lantern.glb",
    category: "lamp",
    mountType: "ceiling",
    defaultScale: 1,
    yOffset: 0,
  ...
  ph_lamp_desk: {
    file: "ph_lamp_desk.glb",
    category: "lamp",
    mountType: "surface",
    defaultScale: 1,
    yOffset: 0,
    fit: "surface",
    // GLB natural aspect is portrait (Z>X) but catalog default is landscape.
    autoOrientToFootprint: true,
  },
  ph_lamp_industrial: {
    file: "ph_lamp_industrial.glb",
    category: "lamp",
    mountType: "ceiling",
    defaultScale: 1,
    yOffset: 0,
  ...
  ph_lamp_pipe: {
    file: "ph_lamp_pipe.glb",
    category: "lamp",
    mountType: "floor",
    defaultScale: 1,
    yOffset: 0,
    fit: "footprint",
    // GLB natural aspect is portrait. Catalog defaults are landscape.
    autoOrientToFootprint: true,
  },
  ph_bed_gothic: {
    file: "ph_bed_gothic.glb",
    category: "bed",
    mountType: "floor",
    defaultScale: 1,
    yOffset: 0,
  ...
  ph_nightstand_classic: {
    file: "ph_nightstand_classic.glb",
    category: "nightstand",
    mountType: "floor",
    defaultScale: 1,
    yOffset: 0,
  ...
  kn_kitchen_bar: {
    // Kenney counter-run cabinet. Without this entry the catalog item rendered
    // as a procedural box instead of the real Kenney model.
    file: "kn_kitchen_bar.glb",
    category: "kitchen",
    mountType: "floor",
    defaultScale: 1,
    yOffset: 0,
  ...
  kn_kitchen_corner_inner: {
    file: "kn_kitchen_corner_inner.glb",
    category: "kitchen",
    mountType: "floor",
    defaultScale: 1,
    yOffset: 0,
  ...
  kn_kitchen_corner_round: {
    file: "kn_kitchen_corner_round.glb",
    category: "kitchen",
    mountType: "floor",
    defaultScale: 1,
    yOffset: 0,
  ...
  kitchen_sink: {
    ...
    wallFacingMode: "free",
    defaultFacing: "forward",
    // Sink catalog default (3 ft wide × 0.65 ft deep) is much wider than the
    // square GLB. Auto-orient swaps if the natural aspect doesn't match.
    autoOrientToFootprint: true,
  },
  kitchen_dishwasher: {
    file: "cabinet.glb",
    ...
    wallFacingMode: "free",
    defaultFacing: "forward",
    // Reuses the cabinet.glb which is wider in X than Z, but the catalog
    // defaults a dishwasher to portrait (1.5 × 1.8). Auto-orient resolves.
    autoOrientToFootprint: true,
  },
  bathroom_vanity_single: {
    file: "bathroom_vanity_single.glb",
    category: "bathroom",
    mountType: "floor",
    defaultScale: 1,
    yOffset: 0,
  ...
  bathroom_vanity_double: {
    file: "bathroom_vanity_double.glb",
    category: "bathroom",
    mountType: "floor",
    defaultScale: 1,
    yOffset: 0,
...
Object.entries(MODEL_REGISTRY).forEach(([key, reg]) => {
  reg.yawOffset = Number.isFinite(reg.yawOffset)
    ? reg.yawOffset
    : Number.isFinite(reg.defaultRotation)
      ? reg.defaultRotation
      : 0;
...
async function loadAll() {
  ...
  if (window.cloudSync && typeof window.cloudSync.onLoad === "function") {
    try {
      ...
      window.reportRoseError?.("loadAll-cloud-merge", err);
    }
  }
}
async function saveAll() {
  ...
  if (window.cloudSync && typeof window.cloudSync.afterSave === "function") {
    window.cloudSync
      .afterSave()
      .catch((err) => window.reportRoseError?.("saveAll-cloud-push", err));
  }
}
...
function esc(s) {
  return window.RoseHTML.escape(s);
}
function updateDebugBadge() {
  const el = document.getElementById("debugBadge");
  if (!el) return;
  const ok = MODEL_DEBUG.ok.size,
    fail = MODEL_DEBUG.fail.size,
    blocked = MODEL_DEBUG.blocked.size;
...
function trackModelStatus(kind, key, file) {
  if (kind === "ok") {
    MODEL_DEBUG.fail.delete(key);
    MODEL_DEBUG.blocked.delete(key);
  }
  if (kind === "fail") {
    MODEL_DEBUG.ok.delete(key);
  }
...
function updateRoomDebugBadge() {
  ...
  if (!el) return;
  const ok = ROOM_MODEL_DEBUG.ok.size,
    fail = ROOM_MODEL_DEBUG.fail.size,
    blocked = ROOM_MODEL_DEBUG.blocked.size,
    active = ROOM_MODEL_DEBUG.active.size;
  const show = active > 0 || ok > 0 || fail > 0 || blocked > 0;
  el.classList.toggle("show", show);
  el.classList.toggle("fail", fail > 0 || blocked > 0);
  el.classList.toggle("ok", show && fail === 0 && blocked === 0);
  el.textContent = show
    ? `Room models ${ok} ok / ${fail} fail / ${blocked} blocked${active ? ` / ${active} mapped` : ""}`
    : "";
}
function trackRoomModelStatus(kind, key) {
  if (kind === "ok") {
    ROOM_MODEL_DEBUG.fail.delete(key);
    ROOM_MODEL_DEBUG.blocked.delete(key);
  }
  if (kind === "fail") {
    ROOM_MODEL_DEBUG.ok.delete(key);
  }
...
async function preflightModelFile(file) {
  if (location.protocol === "file:") {
    return {
      ...
      status: 0,
      ok: false,
      error: `File mode blocked. Open via ${getLocalAppUrl()}`,
    };
  }
...
async function preflightAllModels() {
  const entries = await Promise.all(
    Object.entries(MODEL_REGISTRY).map(async ([key, reg]) => ({
      key,
      file: reg.file,
      ...(await preflightModelFile(reg.file)),
    })),
  );
...
function updateAssetPreflightPanel(entries) {
  ...
  const rows =
    entries && entries.length
      ? entries
      : [...Object.entries(MODEL_REGISTRY)].map(([key, reg]) => {
          ...
          return {
            ...
            file: reg.file,
            url,
            status: pending ? "pending" : (cached && cached.status) || "pending",
            ok: !pending && !!(cached && cached.ok),
            error: pending ? "" : (cached && cached.error) || "",
          };
        });
...
function togglePreflightPanel() {
  preflightPanelOpen = !preflightPanelOpen;
  updateAssetPreflightPanel();
}
...
function updateRoomRuntimeDiag() {
  ...
  const hasAlert = items.some(
    (item) => item.status === "fail" || item.status === "blocked" || item.issues.length,
  );
  el.classList.toggle("on", runtimeDiagOpen || hasAlert);
  const summaryText = summary
    ? `ok ${summary.ok} / fail ${summary.fail} / invisible/offscreen ${summary.invisible} / blocked ${summary.blocked}`
    : "live asset placement diagnostics";
...
function renderRoomRuntimeDiagPanel(el, summaryText, items) {
  ...
  items.forEach((item) => {
    const status = ["ok", "fail", "blocked", "loading"].includes(item.status)
      ? item.status
      : "fail";
...
function toggleRoomRuntimeDiag() {
  runtimeDiagOpen = !runtimeDiagOpen;
  updateRoomRuntimeDiag();
}
...
function analyzeRoomModelPlacement(entry, r) {
  ...
  const issues = [];
  if (size.x < 0.05 || size.y < 0.05 || size.z < 0.05) issues.push("near-zero scale");
  if (
    center.distanceTo(new THREE.Vector3(getRoomFocus(r).x, r.height * 0.3, -getRoomFocus(r).y)) >
    Math.max(18, getRoomFocus(r).maxD * 4.4)
  )
    issues.push("far from room");
  if (box.max.y < 0 || box.min.y < -0.12) issues.push("below floor");
  if (!boxInCameraFrustum(box)) issues.push("offscreen");
  entry.issues = issues;
  return entry;
}
function runRoomModelAudit() {
  if (!is3D || !curRoom || !scene) {
    toast("Enter 3D first");
    return;
  }
  let ok = 0,
    fail = 0,
  ...
  const invisibleKeys = items
    .filter((entry) => entry.status === "ok" && entry.issues.length)
    .map((entry) => entry.key);
    ...
    JSON.stringify(
      {
        ...
        failingKeys,
        invisibleKeys,
        blockedKeys: items.filter((entry) => entry.status === "blocked").map((entry) => entry.key),
      },
      null,
      2,
    ),
  ...
  if (fail || blocked || invisible) {
    console.table(
      items.map((entry) => ({
        ...
        path: entry.file,
        world: formatVec3(entry.worldPosition),
        bbox: formatSize3(entry.bboxSize),
      })),
    );
  }
...
function focusSelectedAsset() {
  ...
  cDist = Math.max(
    ...
    Math.min(
      18,
      Math.max(entry.bboxSize?.x || 3, entry.bboxSize?.y || 3, entry.bboxSize?.z || 3) * 3.2,
    ),
  );
...
function loadSingleModelUrl(url) {
  return new Promise((resolve) => {
    ensureGLTFLoader().load(
      url,
      (gltf) => resolve({ scene: gltf.scene || null, url, error: null }),
      () => {},
      (err) => resolve({ scene: null, url, error: err || new Error("load failed") }),
    );
  });
}
async function resolveAndLoadModelAsset(assetKey) {
  ...
  if (!preflight.ok) {
    return {
      scene: null,
      url,
      error: new Error(`Preflight ${preflight.status || 0} ${preflight.error || ""}`.trim()),
    };
  }
...
function normalizeRoom(room) {
  if (!room) return room;
  room.schemaVersion = Number.isFinite(room.schemaVersion)
    ? room.schemaVersion
    : window.RoseProjectSchema?.APP_SCHEMA_VERSION || 2;
  ...
  room.dimensionAnnotations = Array.isArray(room.dimensionAnnotations)
    ? room.dimensionAnnotations
    : [];
  ...
  const wallPreset =
    WALL_PALETTES.find((w) => w.id === room.materials.wallFinish) || WALL_PALETTES[0];
  room.materials.wallFinish = wallPreset.id;
  room.materials.wall = normalizeColorValue(
    room.materials.wall || wallPreset.color,
    wallPreset.color,
  );
  ...
  room.materials.floor = normalizeColorValue(
    room.materials.floor || floorPreset.color,
    floorPreset.color,
  );
  ...
  room.materials.ceilingBrightness = Number.isFinite(room.materials.ceilingBrightness)
    ? room.materials.ceilingBrightness
    : 1;
  ...
  room.materials.lightCharacter = Number.isFinite(room.materials.lightCharacter)
    ...
    : ({
        ...
        moody: 0.92,
        bright_studio: 0.28,
      }[room.materials.lightingPreset] ?? 0.5);
  ...
  if (room.layerVisibility && typeof room.layerVisibility === "object") {
    Object.keys(PLAN_LAYER_DEFAULTS).forEach((key) => {
      if (typeof room.layerVisibility[key] === "boolean")
        nextLayerVisibility[key] = room.layerVisibility[key];
    });
  }
  room.layerVisibility = nextLayerVisibility;
  room.projectId = room.projectId || room.id;
  room.projectName =
    typeof room.projectName === "string" && room.projectName.trim()
      ? room.projectName.trim()
      : room.name || "Home Project";
  ...
  room.connections = Array.isArray(room.connections)
    ? room.connections
        .map((link) => ({
          roomId: link?.roomId || "",
          side: link?.side || "",
  ...
  room.wallThickness = room.wallThickness || 0.5;
  ...
  room.dimensionAnnotations = room.dimensionAnnotations.map((note) => ({
    ...
    offset: Number.isFinite(note?.offset) ? Math.max(0.3, Math.min(2.5, note.offset)) : 0.8,
  }));
...
function normalizeReferenceOverlay(ref, room) {
  ...
  const pdfPageCount =
    Number.isFinite(ref?.pdfPageCount) && ref.pdfPageCount > 0 ? Math.round(ref.pdfPageCount) : 0;
  return {
    ...
    opacity: Math.max(0.08, Math.min(0.95, Number(ref?.opacity) || 0.56)),
    ...
    scale: Number.isFinite(ref?.scale) ? Math.max(0.1, Math.min(12, ref.scale)) : 1,
    naturalWidth,
    naturalHeight,
    calibrationPoints: Array.isArray(ref?.calibrationPoints)
      ? ref.calibrationPoints.filter(Boolean).slice(0, 2)
      : [],
...
function normalizeColorValue(value, fallback) {
  try {
    return "#" + safeThreeColor(value, fallback).getHexString();
  } catch (_) {
