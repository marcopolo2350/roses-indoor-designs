...
function showP(){
  ...
  if(!sel.type||sel.idx<0){
    ...
    h+=propSection('Home Plan',`<label>PROJECT NAME</label><input value="${esc(currentProjectName())}" onchange="renameCurrentProject(this.value)"><label style="margin-top:8px">CURRENT ROOM</label><input value="${esc(r.name||'Room')}" onchange="renameCurrentRoom(this.value)"><div class="prop-state">Project has <strong>${projectMainRooms(r).length} rooms</strong> across <strong>${floors.length} floor${floors.length===1?'':'s'}</strong>. Exports still use the current room and option.</div><label style="margin-top:8px">FLOORS</label><div class="mat-grid tall">${floors.map(floor=>`<button class="mat-btn${currentFloorId===floor.id?' sel':''}" onclick="setActiveFloor('${floor.id}')">${esc(floor.label)} · ${floor.rooms.length}</button>`).join('')}</div><div class="quick-rotate-row"><button class="pbtn soft" onclick="openAddRoomModalForProject('${currentFloorId}')">Add Room</button><button class="pbtn soft" onclick="createNextFloor()">Add Floor</button>${floors.length>1?`<button class="pbtn soft" onclick="moveCurrentRoomToFloor('${floors.find(floor=>floor.id!==currentFloorId)?.id||currentFloorId}')">Move Room</button>`:''}</div><div class="quick-rotate-row"><button class="pbtn soft" onclick="moveCurrentRoomOrder(-1)">Move Up</button><button class="pbtn soft" onclick="moveCurrentRoomOrder(1)">Move Down</button><button class="pbtn soft" onclick="duplicateCurrentRoom()">Duplicate Room</button></div><div class="room-card-stack">${roomCards||'<div class="prop-tip">No rooms on this floor yet.</div>'}</div><div class="prop-tip">Connected rooms now live as separate rooms inside one project, grouped by floor so whole-home planning stays organized.</div>`);
